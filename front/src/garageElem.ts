...
import { ICar, IEngine } from './inefaces';
import {startEngine, stopEngine, DriveMode} from './functions';
import './styles/garage.css';

export class GarageElem extends MyElement {
  start: (carId: number) => void = () => {};
  stop: () => void = () => {};
  remove: (carId: number) => void = () => {};
  change: (carId: number) => void = () => {};

  private startButton: Button;
  private stopButton: Button;
  private imageCar: MyElement;
  public car: ICar;
  public speed = 0;
  private animation: Animation | undefined;

  constructor(parentNode: HTMLElement, car: ICar) {
    super(parentNode, 'div', ['race__elem']);
    this.car = car;
    const top = new MyElement(this.element, 'div', ['race__elem_top']);
    const select = new Button(top.element, ['lil-button'], 'select');
    select.onClickButton = () => {
      if (car.id) this.change(car.id);
    };

    const remove = new Button(top.element, ['lil-button'], 'remove');
    remove.onClickButton = () => {
      if (car.id) this.remove(car.id);
      this.destruct();
    };
    const carName = new MyElement(top.element, 'span', [], car.name);
    const field = new MyElement(this.element, 'div', ['race__elem_main']);
    const controler = new MyElement(field.element, 'div', ['race__elem_main_engine']);
    this.startButton = new Button(controler.element, ['lil-button'],'A');
    this.startButton.onClickButton = () => {
      if (car.id) this.startEngine(car.id);
    };
    this.stopButton = new Button(controler.element, ['lil-button'], 'B', true);
    this.stopButton.onClickButton = () => {
      if (car.id) this.stopEngine(car.id);
    };
    this.imageCar = new MyElement(field.element, 'div', ['race__elem_main_car']);
    this.imageCar.element.innerHTML = `
    <svg version="1.2" baseProfile="tiny-ps" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 569" width="120">
    <style>
...
130 22 193 51 l64 29 -19 23 c-65 82 -198 227 -209 227 -7 0 -15 -4 -19 -9z"/>
    ...
    const finish = new MyElement(field.element, 'img', ['race__elem_main_finish']);
    finish.element.setAttribute('src', './assets/racing-finish-svgrepo-com.svg');
  }

  changeBtn(type = false): void {
    this.startButton.Disable(type);
    this.stopButton.Disable(!type);
  }

  async stopEngine(carId: number): Promise<void> {
    const res = await stopEngine(carId);
    if (res.status === 200) {
      this.changeBtn();
      this.speed = 0;
      this.animation?.cancel();
      this.imageCar.element.style.left = '60px';
    }
  }

  async startEngine(carId: number): Promise<void> {
    const data = await startEngine(carId);
    if (data.status === 200) {
      this.changeBtn(true);
      const { result: res } = data;
      const time = res.distance / res.velocity;
      this.animationCar(time);
      await this.driveSwitch(res);
    }
  }

  private animationCar(time: number): void {
    this.animation = this.imageCar.element.animate([{ left: '60px' }, { left: `calc(100% - 120px)` }],
      {
        duration: time,
        easing: 'ease-in-out',
      },
    ...
    this.animation.onfinish = () => {
      this.imageCar.element.style.left = `calc(100% - 120px)`;
    };
  }

  private async driveSwitch(car: IEngine): Promise<void> {
    const driveMode = await DriveMode(this.car.id);
